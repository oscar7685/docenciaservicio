...
@NamedQueries({
    ...
    @NamedQuery(name = "Respuestas.findResultadosxPreguntaxEncuestaxProcesoxValor1", query = "SELECT SUM(CASE WHEN r.respuesta = 1 THEN 1 ELSE 0 END) AS Result1, SUM(CASE WHEN r.respuesta = 2 THEN 1 ELSE 0 END) AS Result2, SUM(CASE WHEN r.respuesta = 3 THEN 1 ELSE 0 END) AS Result3, SUM(CASE WHEN r.respuesta = 4 THEN 1 ELSE 0 END) AS Result4, SUM(CASE WHEN r.respuesta = 5 THEN 1 ELSE 0 END) AS Result5, SUM(CASE WHEN r.respuesta = 0 THEN 1 ELSE 0 END) AS Result0 FROM Respuestas r WHERE r.preguntaidPregunta=:pregunta and r.encabezadoidEncabezado.procesoIdproceso=:proceso"),
    @NamedQuery(name = "Respuestas.findResultadosxPreguntaxEncuestaxProcesoxValor5", query = "SELECT SUM(CASE WHEN r.respuesta =:valor THEN 1 ELSE 0 END) FROM Respuestas r WHERE r.preguntaidPregunta=:pregunta and r.encabezadoidEncabezado.procesoIdproceso=:proceso"),
    @NamedQuery(name = "Respuestas.findResultadosxPreguntaxEncuestaxProcesoxValor6", query = "SELECT SUM(CASE WHEN r.respuesta =:valor THEN 1 ELSE 0 END) FROM Respuestas r WHERE r.preguntaidPregunta=:pregunta and  r.encabezadoidEncabezado.cuestionarioidCuestionario=:cuestionario and r.encabezadoidEncabezado.procesoIdproceso=:proceso"),
