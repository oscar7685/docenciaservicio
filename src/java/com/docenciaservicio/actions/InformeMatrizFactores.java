...
public class InformeMatrizFactores implements Action {
    ...
    public String procesar(HttpServletRequest request) throws IOException, ServletException {
        ...
        List<Factor> factores = factorFacade.findAll();
        ...
        for (int i2 = 0; i2 < factores.size(); i2++) {
            ...
            for (int j = 0; j < caracteristicas.size(); j++) {
                ...
                if (sumaCumplimientoCriterios > 0) {
                    cumplimientoC2[j] = (float) sumaCumplimientoCriterios / calificados;
                    cumplimientoC2[j] = (float) (Math.rint(cumplimientoC2[j] * 10) / 10);
                }

            }  // termina de calificar las caracteristicas



            int caracteristicasEvaluadas = 0;
            for (int i = 0; i < factores.get(i2).getCaracteristicaList().size(); i++) {
                if (cumplimientoC2[i] != 0) {
                    caracteristicasEvaluadas++;
                    suma2 += cumplimientoC2[i];
                }
            }
            if (caracteristicasEvaluadas != 0) {
                cumplimientoF[i2] = suma2 / caracteristicasEvaluadas;
        ...
        return "/WEB-INF/vista/proceso/informe/matrizFactores.jsp";
    }
