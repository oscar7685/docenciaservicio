...
<script type="text/javascript">
    ...
    renderTo: 'grafica',
            ...
            title: {
    text: '${criterios.get(0).caracteristicaidCaracteristica.nombreC}'
    },
            xAxis: {
    categories: [
    <c:forEach items="${criterios}" var="criterio2" varStatus="status2">
        <fmt:parseNumber var="cum4"  value="${cumplimientoIN[status2.index]}" />
        <c:choose>
            <c:when test="${cum4>0}">
                <c:choose>
                    <c:when test="${criterios.size()!=status2.index+1}">
                    '${criterio2.idCriterio}' + '-' + '${criterio2.descripcion}',</c:when>                    <c:otherwise>
                                '${criterio2.idCriterio}' + '-' + '${criterio2.descripcion}'
                    </c:otherwise>
                </c:choose>    
            </c:when>             
        </c:choose>
        ...
        name: 'Caracteristicas',
                data: [
    <c:forEach items="${criterios}" var="caracteristica4" varStatus="status33">
        <fmt:parseNumber var="cum5"  value="${cumplimientoIN[status33.index]}" />
        <c:choose>
            <c:when test="${cum5>0}">
                <c:choose>
                    <c:when test="${criterios.size()!=status33.index+1}">
                        <c:choose>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div id="conte" class="span10">
                        <legend>Caracter&iacute;stica: ${caracteristica.nombreC}</legend>
                        <ul class="breadcrumb">
                            ...
                            <c:when test="${criterios.size()!= 0}">
                                ...
                                <table class="table table-striped table-bordered table-condensed inicial">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${criterios}" var="indicador" varStatus="iter">
                                            <fmt:parseNumber var="cum"  value="${cumplimientoIN[iter.index]}" />
                                            <c:choose>
                                                <c:when test="${cum>0}"> 
                                                    <tr>
                                                        <td style="text-align: left">   
                                                            ${indicador.idCriterio}
                                                        </td>
                                                        <td style="text-align: left">   
                                                            <a href="#detalleCriterio&id=${indicador.idCriterio}" data="${indicador.descripcion}">${indicador.descripcion}</a> 
                                                        </td>
